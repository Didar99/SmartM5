...
public class MainActivity extends Activity {
    ...
    GridView gridView;
    String [] numberWord = {"Çyra", "Howa sazlaýjy", "Rozetka", "Perde", "Telewizor", "Rozetka Aşhana", "Gorag Ulgamy", "Rozetka Otag 1", "Elektrik peç", "Bellikler"};

    String [] numberSubWord = {"Ýagtylandyryş, Görnüş", "Temperatura, Tizlik", "Ýagdaý", "Dereje", "Görnüş", "Ýagdaý",
            "Tertip, Funksiýa", "Ýagdaý", "Ojak A, Ojak B", ""};

    int[] numberImage = {R.drawable.ic_lamp, R.drawable.conditioner, R.drawable.ic_socket, R.drawable.ic_blind,
            R.drawable.ic_tv, R.drawable.ic_socket, R.drawable.secure, R.drawable.ic_socket, R.drawable.ic_oven, R.drawable.ic_info};

    // All device states 0 or 1
    int light_state1, light_state2, light_state3,
            air_state, SpeedAir1_state, SpeedAir2_state,
            SpeedAir3_state, socket1_state, socket2_state,
            socket3_state, water_valve, water_leak_sensor,
            gerKon_sensor, door_sensor, watt_value, water_value,
            gas_leak_sensor, gas_valve, pir_sensor, fire_sensor, all_electric_device, kitchen_state, dimmer_light, stove1_state, stove2_state;   // tv_state
    // water, gas, PIR, gerKon => [BOOLEAN] (FOR -> call only one SMS or NOTIFICATION)
    boolean bool_water, bool_gas, bool_pir, bool_gerKon, bool_fire = false;
...
//        strSecondParam = PrefConfig.loadSecondParam(this);
        ...
        txtDEV.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
...
//        if (socket1_state == 1) {
//
//        } else if (socket1_state == 0) {
//
//        } else if (socket2_state == 1) {
//
//        } else if (socket2_state == 0) {
//
//        } else if (socket3_state == 1) {
//
//        } else if (socket3_state == 2) {
//
//        }
...
//            txt_waterValue.setText(String.valueOf(water_value));
            ...
            // smart stove
            JSONObject obj6 = jArr.getJSONObject(6);
            JSONArray pinStove1 = obj6.getJSONArray("pins");
            JSONArray pinStove2 = obj6.getJSONArray("pins");
            // => socket1 state 1 or 0
            JSONObject Stove1 = pinStove1.getJSONObject(1);
            stove1_state = Stove1.getInt("action");
            // => socket2 state 1 or 0
            JSONObject Stove2 = pinStove2.getJSONObject(3);
            stove2_state = Stove2.getInt("action");

            PrefConfig.saveStateStove(getApplicationContext(), stove1_state);
            PrefConfig.saveStateStove2(getApplicationContext(), stove2_state);
...
//            PrefConfig.saveAirSpeed3(getApplicationContext(), SpeedAir3_state);
            ...
            JSONObject obj11 = jArr.getJSONObject(24); //This index takes from (devicesConfig.py)->device rows(ex:water sensor located in 12 rows of devices and you must take n-1) (FireFox best solution for see)
            ...
            // door sensor data 1 or 0 (open or close)
            ...
            // FIRE sensor
            JSONObject obj19 = jArr.getJSONObject(19);
            fire_sensor = obj19.getInt("state");

            // All electric device off or on
            JSONObject obj21 = jArr.getJSONObject(21);
            all_electric_device = obj21.getInt("state");
            PrefConfig.saveElectricDevices(getApplicationContext(), all_electric_device);

            // kitchen_light123 device off or on
            JSONObject obj22 = jArr.getJSONObject(22);
            kitchen_state = obj22.getInt("state");
            PrefConfig.saveKitchenState(getApplicationContext(), kitchen_state);


            // dimmer light 3 steps
            JSONObject obj23 = jArr.getJSONObject(23);
            dimmer_light = obj23.getInt("state");
            PrefConfig.saveDimmerState(getApplicationContext(), dimmer_light);
...
//            System.out.println("AirSpd3 state   => " + SpeedAir3_state);
            System.out.println("FIRE sensor     => " + fire_sensor);
            System.out.println("ELECTRIC_DEVICE => " + all_electric_device);
            System.out.println("KITCHEN STATE   => " + kitchen_state);
            System.out.println("DIMMER STATE    => " + dimmer_light);
...
//            if (gas_valve == 1) { } else if (gas_valve == 0) { }
            ...
            if (water_leak_sensor == 1 && !bool_water) {
                ...
                PrefConfig.saveWaterValve(getApplicationContext(), water_valve);
            ...
            if (gerKon_sensor == 0 && bool_gerKon) { bool_gerKon = false;} else if (gerKon_sensor == 1 && !bool_gerKon) {
                gerKon_sensor_off(getApplicationContext());
                if (sms_activator == 1) { smsManager.sendTextMessage("+993" + phoneNumber, null, "Gapy açyldy!", null, null); }
...
//                if (pir_activator == 1) { pir_light_off(getApplicationContext()); }
...
//                if (pir_activator == 1) { pir_light_on(getApplicationContext()); }
//                bool_pir = true;
//            }
            ...
            if (fire_sensor == 1 && !bool_fire) {
                ...
                bool_fire = false;
            } else if (fire_sensor == 0 && bool_fire) {
                bool_fire = true;
            }
        }
    }
    ...
    private void water_note() {
        AlertDialog dialog = new AlertDialog.Builder(MainActivity.this)
                .setTitle("Suw agdy")
                .setMessage("Esasy suw bekediji ýapyldy")
                .setCancelable(true)
                .create();
        dialog.show();
    }

    private void fire_note() {
        AlertDialog dialog = new AlertDialog.Builder(MainActivity.this)
                .setTitle("Ýangyn!")
                .setMessage("Ýagdaýy dolandyrmak üçin basyň")
                .setCancelable(true)
                .create();
        dialog.show();
    }
...
//            AlertDialog dialog = new AlertDialog.Builder(MainActivity.this)
//                    .setTitle("Sazlamalar")
//                    .setMessage("Sazlamalar üýtgedildi. Sahypa täzelener!")
//                    .setCancelable(false)
//                    .setPositiveButton("Tassykla", (dialogInterface, i) -> finishAffinity())
//                    .create();
//            dialog.show();
//        }
//        sd.start(sm);
